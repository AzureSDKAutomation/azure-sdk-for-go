...
type AccountCreateProperties struct {
	...
	// PublicNetworkAccess - The network access type for accessing Azure Batch account. If not specified, the default value is 'enabled'. Possible values include: 'PublicNetworkAccessTypeEnabled', 'PublicNetworkAccessTypeDisabled'
	PublicNetworkAccess PublicNetworkAccessType `json:"publicNetworkAccess,omitempty"`
	// Encryption - The encryption configuration for the Batch account. Configures how customer data is encrypted inside the Batch account. By default, accounts are encrypted using a Microsoft managed key. For additional control, a customer-managed key can be used instead.
...
type AccountProperties struct {
	...
	// PoolAllocationMode - READ-ONLY; The allocation mode to use for creating pools in the Batch account. Possible values include: 'BatchService', 'UserSubscription'
	PoolAllocationMode PoolAllocationMode `json:"poolAllocationMode,omitempty"`
	// KeyVaultReference - READ-ONLY; A reference to the Azure key vault associated with the Batch account.
	KeyVaultReference *KeyVaultReference `json:"keyVaultReference,omitempty"`
	// PublicNetworkAccess - READ-ONLY; The network interface type for accessing Azure Batch service and Batch account operations. If not specified, the default value is 'enabled'. Possible values include: 'PublicNetworkAccessTypeEnabled', 'PublicNetworkAccessTypeDisabled'
	PublicNetworkAccess PublicNetworkAccessType `json:"publicNetworkAccess,omitempty"`
	...
	// AutoStorage - READ-ONLY; The properties and status of any auto-storage account associated with the Batch account.
	AutoStorage *AutoStorageProperties `json:"autoStorage,omitempty"`
	// Encryption - READ-ONLY; The encryption configuration for the Batch account. Configures how customer data is encrypted inside the Batch account. By default, accounts are encrypted using a Microsoft managed key. For additional control, a customer-managed key can be used instead.
	Encryption *EncryptionProperties `json:"encryption,omitempty"`
	// DedicatedCoreQuota - READ-ONLY; The dedicated core quota for the Batch account. For accounts with PoolAllocationMode set to UserSubscription, quota is managed on the subscription so this value is not returned.
	DedicatedCoreQuota *int32 `json:"dedicatedCoreQuota,omitempty"`
	// LowPriorityCoreQuota - READ-ONLY; The low-priority core quota for the Batch account. For accounts with PoolAllocationMode set to UserSubscription, quota is managed on the subscription so this value is not returned.
	...
	// DedicatedCoreQuotaPerVMFamilyEnforced - READ-ONLY; A value indicating whether core quotas per Virtual Machine family are enforced for this account. Batch is transitioning its core quota system for dedicated cores to be enforced per Virtual Machine family. During this transitional phase, the dedicated core quota per Virtual Machine family may not yet be enforced. If this flag is false, dedicated core quota is enforced via the old dedicatedCoreQuota property on the account and does not consider Virtual Machine family. If this flag is true, dedicated core quota is enforced via the dedicatedCoreQuotaPerVMFamily property on the account, and the old dedicatedCoreQuota does not apply.
	DedicatedCoreQuotaPerVMFamilyEnforced *bool `json:"dedicatedCoreQuotaPerVMFamilyEnforced,omitempty"`
	// PoolQuota - READ-ONLY; The pool quota for the Batch account.
	PoolQuota *int32 `json:"poolQuota,omitempty"`
	// ActiveJobAndJobScheduleQuota - READ-ONLY; The active job and job schedule quota for the Batch account.
...
type AutoScaleSettings struct {
	...
	// EvaluationInterval - The time interval at which to automatically adjust the pool size according to the autoscale formula. If omitted, the default value is 15 minutes (PT15M).
...
type AutoStorageBaseProperties struct {
	...
	// NodeIdentityReference - The reference to the user assigned identity which compute nodes will use to access auto-storage. The identity referenced here must be assigned to pools which have compute nodes that need access to auto-storage.
...
type CertificateBaseProperties struct {
	// ThumbprintAlgorithm - The algorithm of the certificate thumbprint. This must match the first portion of the certificate name. Currently required to be 'SHA1'.
	ThumbprintAlgorithm *string `json:"thumbprintAlgorithm,omitempty"`
	// Thumbprint - The thumbprint of the certificate. This must match the thumbprint from the name.
...
type CertificateCreateOrUpdateProperties struct {
	...
	// Password - The password to access the certificate's private key. This must not be specified if the certificate format is Cer.
...
type CertificateProperties struct {
	// ProvisioningState - READ-ONLY; The provisioned state of the resource. Possible values include: 'Succeeded', 'Deleting', 'Failed'
	ProvisioningState CertificateProvisioningState `json:"provisioningState,omitempty"`
	// ProvisioningStateTransitionTime - READ-ONLY; The time at which the certificate entered its current state.
	...
	// PreviousProvisioningStateTransitionTime - READ-ONLY; The time at which the certificate entered its previous state.
	...
	// DeleteCertificateError - READ-ONLY; The error which occurred while deleting the certificate. This is only returned when the certificate provisioningState is 'Failed'.
...
// CertificateReference a reference to a certificate to be installed on compute nodes in a pool. This must
...
type CertificateReference struct {
	...
	ID *string `json:"id,omitempty"`
	// StoreLocation - The location of the certificate store on the compute node into which to install the certificate. The default value is currentUser. This property is applicable only for pools configured with Windows nodes (that is, created with cloudServiceConfiguration, or with virtualMachineConfiguration using a Windows image reference). For Linux compute nodes, the certificates are stored in a directory inside the task working directory and an environment variable AZ_BATCH_CERTIFICATES_DIR is supplied to the task to query for this location. For certificates with visibility of 'remoteUser', a 'certs' directory is created in the user's home directory (e.g., /home/{user-name}/certs) and certificates are placed in that directory. Possible values include: 'CurrentUser', 'LocalMachine'
	StoreLocation CertificateStoreLocation `json:"storeLocation,omitempty"`
	// StoreName - The name of the certificate store on the compute node into which to install the certificate. This property is applicable only for pools configured with Windows nodes (that is, created with cloudServiceConfiguration, or with virtualMachineConfiguration using a Windows image reference). Common store names include: My, Root, CA, Trust, Disallowed, TrustedPeople, TrustedPublisher, AuthRoot, AddressBook, but any custom store name can also be used. The default value is My.
...
type DataDisk struct {
	// Lun - The logical unit number. This is used to uniquely identify each data disk. If attaching multiple disks, each should have a distinct lun. The value must be between 0 and 63, inclusive.
	Lun *int32 `json:"lun,omitempty"`
	// Caching - The type of caching to be enabled for the data disks. Values are:
	...
	// StorageAccountType - The storage account type to be used for the data disk. If omitted, the default is "Standard_LRS". Values are:
...
// DiffDiskSettings specifies the ephemeral Disk Settings for the operating system disk used by the virtual
// machine.
...
type DiskEncryptionConfiguration struct {
	// Targets - The list of disk targets Batch Service will encrypt on the compute node. On Linux pool, only "TemporaryDisk" is supported; on Windows pool, "OsDisk" and "TemporaryDisk" must be specified.
...
type FixedScaleSettings struct {
	...
	// TargetDedicatedNodes - The desired number of dedicated compute nodes in the pool. At least one of targetDedicatedNodes, targetLowPriorityNodes must be set.
	TargetDedicatedNodes *int32 `json:"targetDedicatedNodes,omitempty"`
	// TargetLowPriorityNodes - The desired number of low-priority compute nodes in the pool. At least one of targetDedicatedNodes, targetLowPriorityNodes must be set.
	TargetLowPriorityNodes *int32 `json:"targetLowPriorityNodes,omitempty"`
	// NodeDeallocationOption - Determines what to do with a node and its running task(s) if the pool size is decreasing. If omitted, the default value is Requeue. Possible values include: 'Requeue', 'Terminate', 'TaskCompletion', 'RetainedData'
	NodeDeallocationOption ComputeNodeDeallocationOption `json:"nodeDeallocationOption,omitempty"`
}

// ImageReference a reference to an Azure Virtual Machines Marketplace image or the Azure Image resource of
// a custom Virtual Machine. To get the list of all imageReferences verified by Azure Batch, see the 'List
...
type NetworkConfiguration struct {
	...
	// PublicIPAddressConfiguration - The Public IPAddress configuration for Compute Nodes in the Batch Pool. This property is only supported on Pools with the virtualMachineConfiguration property.
...
// NodePlacementConfiguration node placement configuration for batch pools. Allocation configuration used
// by Batch Service to provision the nodes.
type NodePlacementConfiguration struct {
	// Policy - The node placement policy used to provision nodes. If not specified, Batch will use the regional policy. Possible values include: 'Regional', 'Zonal'
	Policy NodePlacementPolicyType `json:"policy,omitempty"`
}

// Operation a REST API operation
type Operation struct {
	// Name - The operation name. This is of the format {provider}/{resource}/{operation}
	Name *string `json:"name,omitempty"`
...
type OperationDisplay struct {
	...
	// Operation - The operation type. For example: read, write, delete, or listKeys/action
...
type PoolProperties struct {
	...
	DisplayName *string `json:"displayName,omitempty"`
	// LastModified - READ-ONLY; The last modified time of the pool. This is the last time at which the pool level data, such as the targetDedicatedNodes or autoScaleSettings, changed. It does not factor in node-level changes such as a compute node changing state.
	LastModified *date.Time `json:"lastModified,omitempty"`
	// CreationTime - READ-ONLY; The creation time of the pool.
	CreationTime *date.Time `json:"creationTime,omitempty"`
	// ProvisioningState - READ-ONLY; The current state of the pool. Possible values include: 'PoolProvisioningStateSucceeded', 'PoolProvisioningStateDeleting'
	ProvisioningState PoolProvisioningState `json:"provisioningState,omitempty"`
	// ProvisioningStateTransitionTime - READ-ONLY; The time at which the pool entered its current state.
	ProvisioningStateTransitionTime *date.Time `json:"provisioningStateTransitionTime,omitempty"`
	// AllocationState - READ-ONLY; Whether the pool is resizing. Possible values include: 'Steady', 'Resizing', 'Stopping'
	AllocationState AllocationState `json:"allocationState,omitempty"`
	// AllocationStateTransitionTime - READ-ONLY; The time at which the pool entered its current allocation state.
	AllocationStateTransitionTime *date.Time `json:"allocationStateTransitionTime,omitempty"`
	// VMSize - The size of virtual machines in the pool. All VMs in a pool are the same size. For information about available sizes of virtual machines for Cloud Services pools (pools created with cloudServiceConfiguration), see Sizes for Cloud Services (https://azure.microsoft.com/documentation/articles/cloud-services-sizes-specs/). Batch supports all Cloud Services VM sizes except ExtraSmall. For information about available VM sizes for pools using images from the Virtual Machines Marketplace (pools created with virtualMachineConfiguration) see Sizes for Virtual Machines (Linux) (https://azure.microsoft.com/documentation/articles/virtual-machines-linux-sizes/) or Sizes for Virtual Machines (Windows) (https://azure.microsoft.com/documentation/articles/virtual-machines-windows-sizes/). Batch supports all Azure VM sizes except STANDARD_A0 and those with premium storage (STANDARD_GS, STANDARD_DS, and STANDARD_DSV2 series).
	VMSize *string `json:"vmSize,omitempty"`
	// DeploymentConfiguration - Describes how the pool nodes will be deployed - using Cloud Services or Virtual Machines. Using CloudServiceConfiguration specifies that the nodes should be creating using Azure Cloud Services (PaaS), while VirtualMachineConfiguration uses Azure Virtual Machines (IaaS).
	DeploymentConfiguration *DeploymentConfiguration `json:"deploymentConfiguration,omitempty"`
	// CurrentDedicatedNodes - READ-ONLY; The number of compute nodes currently in the pool.
	CurrentDedicatedNodes *int32 `json:"currentDedicatedNodes,omitempty"`
	// CurrentLowPriorityNodes - READ-ONLY; The number of low-priority compute nodes currently in the pool.
	CurrentLowPriorityNodes *int32 `json:"currentLowPriorityNodes,omitempty"`
	// ScaleSettings - Settings which configure the number of nodes in the pool.
	ScaleSettings *ScaleSettings `json:"scaleSettings,omitempty"`
	// AutoScaleRun - READ-ONLY; The results and errors from the last execution of the autoscale formula. This property is set only if the pool automatically scales, i.e. autoScaleSettings are used.
	...
	// StartTask - A task specified to run on each compute node as it joins the pool. In an PATCH (update) operation, this property can be set to an empty object to remove the start task from the pool.
	StartTask *StartTask `json:"startTask,omitempty"`
	// Certificates - The list of certificates to be installed on each compute node in the pool. For Windows compute nodes, the Batch service installs the certificates to the specified certificate store and location. For Linux compute nodes, the certificates are stored in a directory inside the task working directory and an environment variable AZ_BATCH_CERTIFICATES_DIR is supplied to the task to query for this location. For certificates with visibility of 'remoteUser', a 'certs' directory is created in the user's home directory (e.g., /home/{user-name}/certs) and certificates are placed in that directory.
	...
	// ResizeOperationStatus - READ-ONLY; Contains details about the current or last completed resize operation.
...
type PrivateEndpoint struct {
	// ID - READ-ONLY; The ARM resource identifier of the private endpoint. This is of the form /subscriptions/{subscription}/resourceGroups/{group}/providers/Microsoft.Network/privateEndpoints/{privateEndpoint}.
	ID *string `json:"id,omitempty"`
}
...
type PrivateEndpointConnectionProperties struct {
	// ProvisioningState - READ-ONLY; The provisioning state of the private endpoint connection. Possible values include: 'PrivateEndpointConnectionProvisioningStateSucceeded', 'PrivateEndpointConnectionProvisioningStateUpdating', 'PrivateEndpointConnectionProvisioningStateFailed'
	ProvisioningState PrivateEndpointConnectionProvisioningState `json:"provisioningState,omitempty"`
	// PrivateEndpoint - The ARM resource identifier of the private endpoint.
...
type PrivateLinkResourceProperties struct {
	// GroupID - READ-ONLY; The group id of the private link resource. The group id is used to establish the private link connection.
	GroupID *string `json:"groupId,omitempty"`
	// RequiredMembers - READ-ONLY; The list of required members that are used to establish the private link connection.
	RequiredMembers *[]string `json:"requiredMembers,omitempty"`
	// RequiredZoneNames - READ-ONLY; The list of required zone names for the private DNS resource name
...
type PrivateLinkServiceConnectionState struct {
	// Status - The status for the private endpoint connection of Batch account. Possible values include: 'PrivateLinkServiceConnectionStatusApproved', 'PrivateLinkServiceConnectionStatusPending', 'PrivateLinkServiceConnectionStatusRejected', 'PrivateLinkServiceConnectionStatusDisconnected'
	Status PrivateLinkServiceConnectionStatus `json:"status,omitempty"`
	// Description - Description of the private Connection state
	Description *string `json:"description,omitempty"`
	// ActionRequired - READ-ONLY; Action required on the private connection state
...
// ResizeOperationStatus details about the current or last completed resize operation. Describes either the
// current operation (if the pool AllocationState is Resizing) or the previously completed operation (if
...
type ResizeOperationStatus struct {
	...
	ResizeTimeout *string `json:"resizeTimeout,omitempty"`
	// NodeDeallocationOption - Determines what to do with a node and its running task(s) if the pool size is decreasing. The default value is requeue. Possible values include: 'Requeue', 'Terminate', 'TaskCompletion', 'RetainedData'
	NodeDeallocationOption ComputeNodeDeallocationOption `json:"nodeDeallocationOption,omitempty"`
	// StartTime - The time when this resize operation was started.
	StartTime *date.Time `json:"startTime,omitempty"`
	// Errors - Details of any errors encountered while performing the last resize on the pool. This property is set only if an error occurred during the last pool resize, and only when the pool allocationState is Steady.
...
type ResourceFile struct {
	...
	// BlobPrefix - The blob prefix to use when downloading blobs from an Azure Storage container. Only the blobs whose names begin with the specified prefix will be downloaded. Valid only when autoStorageContainerName or storageContainerUrl is used. This prefix can be a partial filename or a subdirectory. If a prefix is not specified, all the files in the container will be downloaded.
	...
	// FileMode - The file permission mode attribute in octal format. This property applies only to files being downloaded to Linux compute nodes. It will be ignored if it is specified for a resourceFile which will be downloaded to a Windows node. If this property is not specified for a Linux node, then a default value of 0770 is applied to the file.
...
// ScaleSettings scale settings for the pool. Defines the desired size of the pool. This can either be
...
type ScaleSettings struct {
	// FixedScale - Fixed scale settings for the pool. This property and autoScale are mutually exclusive and one of the properties must be specified.
	FixedScale *FixedScaleSettings `json:"fixedScale,omitempty"`
	// AutoScale - AutoScale settings for the pool. This property and fixedScale are mutually exclusive and one of the properties must be specified.
...
// StartTask a task which is run when a compute node joins a pool in the Azure Batch service, or when the
// compute node is rebooted or reimaged. In some cases the start task may be re-run even though the node
// was not rebooted. Due to this, start tasks should be idempotent and exit gracefully if the setup they're
...
type StartTask struct {
	// CommandLine - The command line of the start task. The command line does not run under a shell, and therefore cannot take advantage of shell features such as environment variable expansion. If you want to take advantage of such features, you should invoke the shell in the command line, for example using "cmd /c MyCommand" in Windows or "/bin/sh -c MyCommand" in Linux. Required if any other properties of the startTask are specified.
	...
	// UserIdentity - The user identity under which the start task runs. If omitted, the task runs as a non-administrative user unique to the task.
	...
	// ContainerSettings - The settings for the container under which the start task runs. When this is specified, all directories recursively below the AZ_BATCH_NODE_ROOT_DIR (the root of Azure Batch directories on the node) are mapped into the container, all task environment variables are mapped into the container, and the task command line is executed in the container.
...
type TaskContainerSettings struct {
	// ContainerRunOptions - Additional options to the container create command. These options are supplied as arguments to the "docker create" command, in addition to those controlled by the Batch Service.
	...
	// WorkingDirectory - A flag to indicate where the container task working directory is. The default is 'taskWorkingDirectory'. Possible values include: 'TaskWorkingDirectory', 'ContainerImageDefault'
...
type TaskSchedulingPolicy struct {
	// NodeFillType - How tasks should be distributed across compute nodes. Possible values include: 'Spread', 'Pack'
...
type UserAccount struct {
	...
	Password *string `json:"password,omitempty"`
	// ElevationLevel - The elevation level of the user account. nonAdmin - The auto user is a standard user without elevated access. admin - The auto user is a user with elevated access and operates with full Administrator permissions. The default value is nonAdmin. Possible values include: 'NonAdmin', 'Admin'
	ElevationLevel ElevationLevel `json:"elevationLevel,omitempty"`
...
type VirtualMachineConfiguration struct {
	...
	// WindowsConfiguration - Windows operating system settings on the virtual machine. This property must not be specified if the imageReference specifies a Linux OS image.
	WindowsConfiguration *WindowsConfiguration `json:"windowsConfiguration,omitempty"`
	// DataDisks - The configuration for data disks attached to the compute nodes in the pool. This property must be specified if the compute nodes in the pool need to have empty data disks attached to them.
	...
	// ContainerConfiguration - The container configuration for the pool. If specified, setup is performed on each node in the pool to allow tasks to run in containers. All regular tasks and job manager tasks run on this pool must specify the containerSettings property, and all other tasks may specify it.
	...
	// OsDisk - Settings for the operating system disk of the Virtual Machine. Contains configuration for ephemeral OSDisk settings.
...
type WindowsUserConfiguration struct {
	// LoginMode - The login mode for the user. The default value for VirtualMachineConfiguration pools is interactive mode and for CloudServiceConfiguration pools is batch mode. Possible values include: 'Batch', 'Interactive'
